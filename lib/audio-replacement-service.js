...
function isArchivedTrack(track) {
...
function createAudioReplacementService({
  ...
  async function cleanupHardBrokenTracks(options = {}) {
    if (fullAuditState.running) {
      throw new Error("Aspetta la fine della verifica YouTube prima di archiviare il catalogo.");
    }
    ...
    if (removableIds.size === 0) {
      return {
        ok: true,
        message: "Nessuna traccia rotta confermata da archiviare.",
        archived: 0,
        removed: 0,
        kept: items.length,
        backupFile: "",
    ...
    const reasonById = new Map(removableItems.map((item) => [firstString(item.id), firstString(item.reason, "unknown")]));
    let archived = 0;
    const nextLibrary = library.map((track) => {
      const trackId = firstString(track.id);
      if (!removableIds.has(trackId)) {
        return track;
      }
      ...
      return {
        ...
        archivedAt,
        archivedReason: reasonById.get(trackId) || "audio-check-failed",
        archivedSource: "audio-cleanup",
      };
    });
    ...
    return {
      ok: true,
      message: `Quarantena completata: ${archived} tracce YouTube non disponibili nascoste dal catalogo attivo.`,
      archived,
      removed: 0,
  ...
  async function recheckArchivedTracks(options = {}) {
    if (fullAuditState.running) {
      throw new Error("Aspetta la fine della verifica YouTube prima di riverificare le archiviate.");
    }

    if (typeof writeLibrary !== "function") {
      throw new Error("Riverifica archivio non configurata: writeLibrary mancante.");
    }

    const library = await readLibrary();
    const candidates = library.filter((track) => isYouTubeTrack(track) && isArchivedTrack(track));
    const maxTracks = Math.max(1, Math.min(1000, Number(options.limit || env.CLEARWAVE_YOUTUBE_ARCHIVED_RECHECK_LIMIT) || 120));
    const selected = candidates.slice(0, maxTracks);

    if (selected.length === 0) {
      return {
        ok: true,
        message: "Nessuna traccia YouTube archiviata da riverificare.",
        candidates: 0,
        checked: 0,
        restored: 0,
        truncated: false,
    ...
    const result = await runCatalogAudioCheckForIds(
      selected.map((track) => firstString(track.id)).filter(Boolean),
      {
        mode: options.mode || "metadata",
        concurrency: Number(options.concurrency) || 2,
        timeoutMs: Number(options.timeoutMs) || 35000,
        sampleSeconds: Number(options.sampleSeconds) || 4,
    ...
    if (restoredIds.size > 0) {
      backupFile = await backupLibraryBeforeCleanup(library);
      const restoredAt = new Date().toISOString();
      const nextLibrary = library.map((track) => {
        const trackId = firstString(track.id);
        if (!restoredIds.has(trackId)) {
          return track;
        }
        ...
        const nextTrack = {
          ...
          restoredAt,
          restoredSource: "archived-audio-recheck",
        };
        delete nextTrack.archivedReason;
        delete nextTrack.archivedSource;
        return nextTrack;
      });
      await writeLibrary(nextLibrary);
    }
    ...
    const replacementList = await readList();
    const nextItems = (replacementList.items || []).filter((item) => !restoredIds.has(firstString(item.id)));
    const nextList =
      restored > 0
        ? await writeList({
            ...replacementListFallback(),
            ...replacementList,
            updatedAt: new Date().toISOString(),
            source: "archived-audio-recheck",
            summary: {
              ...(replacementList.summary || {}),
              archivedRecheckChecked: results.length,
              archivedRecheckRestored: restored,
              replaceCount: nextItems.length,
            },
            items: nextItems,
          })
        : replacementList;
    ...
    return {
      ...
      message:
        restored > 0
          ? `Riverifica completata: ${restored} tracce archiviate sono tornate disponibili.`
          : `Riverifica completata: nessuna delle ${results.length} tracce archiviate controllate e' tornata disponibile.`,
  ...
  function reportPathFromValue(value) {
    const raw = firstString(value);
    if (!raw) {
      return "";
    }
    return path.isAbsolute(raw) ? raw : path.join(reportsDir, path.basename(raw));
  }

  async function latestYouTubeReport() {
    const replacementList = await readList();
    const candidates = [
      reportPathFromValue(fullAuditState.reportJson),
      reportPathFromValue(replacementList.reportJson),
      ...(await recentReports(6)).map((report) => report.filePath),
    ].filter(Boolean);
    ...
    for (const filePath of candidates) {
      ...
      seen.add(filePath);
      const report = await readJsonFileIfExists(filePath, null);
      const results = Array.isArray(report?.results) ? report.results : [];
      if (results.some((result) => result?.provider === "youtube")) {
        return { filePath, report };
      }
    }

    return { filePath: "", report: null };
  }

  async function youtubeResults(options = {}) {
    const status = firstString(options.status, "failed").toLowerCase();
    const reason = firstString(options.reason).toLowerCase();
    const offset = boundedNumber(options.offset, 0, 0, 100000);
    const limit = boundedNumber(options.limit, 80, 1, 200);
    const latest = await latestYouTubeReport();
    const report = latest.report;
    const allResults = Array.isArray(report?.results)
      ? report.results.filter((result) => result?.provider === "youtube")
      : [];
    const filtered = allResults.filter((result) => {
      const resultStatus = firstString(result.status).toLowerCase();
      const resultReason = firstString(result.reason).toLowerCase();
      if (status !== "all" && resultStatus !== status) {
        return false;
      }
      if (reason && resultReason !== reason) {
        return false;
      }
      return true;
    });
    ...
    return {
      ok: Boolean(report),
      createdAt: firstString(report?.createdAt),
      hasMore: offset + limit < filtered.length,
      items: filtered.slice(offset, offset + limit).map((result) => ({
        id: firstString(result.id),
        title: firstString(result.title, result.id),
        status: firstString(result.status, "unknown"),
        reason: firstString(result.reason, "unknown"),
        message: firstString(result.message).slice(0, 500),
        source: firstString(result.source),
        durationMs: result.durationMs || 0,
      })),
      limit,
      offset,
      reportJson: latest.filePath ? path.basename(latest.filePath) : "",
      reason,
      status,
      summary: report?.summary || null,
      totalFiltered: filtered.length,
      totalYoutube: allResults.length,
    };
  }
  ...
  async function cookieProbeCandidate() {
    // Il test cookie deve usare una traccia che ha davvero chiesto login, non un video pubblico qualunque.
    const replacementList = await readList();
    const replacementItem = (replacementList.items || []).find((item) => {
      const source = firstString(item?.source);
      return source && /(?:youtube\.com|youtu\.be)/i.test(source) && isYouTubeLoginFailure(item);
    });
    if (replacementItem) {
      return {
        source: replacementItem.source,
        title: firstString(replacementItem.title),
        reason: firstString(replacementItem.reason, "youtube-age-or-login"),
        from: "audio-replacement-list",
      };
    }
    ...
    for (const reportInfo of reports) {
      const report = await readJsonFileIfExists(reportInfo.filePath, null);
      const results = Array.isArray(report?.results) ? report.results : [];
      const reportItem = results.find((result) => {
        const source = firstString(result?.source);
        return result?.provider === "youtube" && source && /(?:youtube\.com|youtu\.be)/i.test(source) && isYouTubeLoginFailure(result);
      });
      if (reportItem) {
        return {
          source: reportItem.source,
          title: firstString(reportItem.title),
          reason: firstString(reportItem.reason, "youtube-age-or-login"),
          from: path.basename(reportInfo.filePath),
        };
      }
    }

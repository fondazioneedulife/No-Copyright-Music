...
| `youtube-stream-open-failed` | `yt-dlp` ha risolto il video, ma `mpv` non e' riuscito ad aprire lo stream `googlevideo.com` firmato (`403`, `Failed to open`, oppure `avformat_open_input`). | Riprova; se succede spesso con cookie validi, configura un PO token yt-dlp o sostituisci le tracce instabili. |
